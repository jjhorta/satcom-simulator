...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const t = p.get("token"); const tf = p.get("twofa");
    if (t) {
      fetch("/api/auth/me", { headers: { Authorization: "Bearer "+t } })
        .then(r => r.json()).then(u => { setToken(t); if (u) setUser(u); navigate("/") }).catch(() => {})
    } else if (tf) {
      setShowTwofa(true)
    }
  }, [])
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-950">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <div className="mt-4">
            <a href="/constellation-simulator/api/auth/google/login"
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-gray-700 bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-white transition-colors text-sm">
